function servicetask14(attempt, message) {
	...
	try{
		...
		destinatario = ['itamar.abe@flamboyant.com.br',
			            'matheus.cavalcante@flamboyant.com.br'
		                ];

		log.warn("------------------------ LOG EMAIL subject: " + subject)
		log.warn("------------------------ LOG EMAIL mailFluig: " + mailFluig)
		log.warn("------------------------ LOG EMAIL destinatario: " + destinatario)

		var htmlMsg = "";
		htmlMsg += "<p>A requisição, número " + processo + ", para aumento de quadro foi aprovada.</p>";
		htmlMsg += "<p>Os processos relativos à admissão deste novo funcionário devem ser iniciados.</p>";

		log.warn("------------------------ LOG EMAIL htmlMsg: " + htmlMsg)

		for (y in destinatario){
			obj.simpleEmail(1,subject, mailFluig, destinatario[y], htmlMsg, "text/html");
		}

			
		var cDsChapa 		= DatasetFactory.createConstraint("MATRICULA", hAPI.getCardValue("chapaRequisitante"), hAPI.getCardValue("chapaRequisitante"), ConstraintType.MUST);
		var dsChapaRM		= DatasetFactory.getDataset("dsGetChapa", null, new Array(cDsChapa), null);
		...
		log.warn("APROVADOR DOIS NO FORMUALRIO ==> " + hAPI.getCardValue("matApDois"))
		log.warn("APROVADOR TRES  NO FORMUALRIO ==> " + hAPI.getCardValue("matApTres"))
		log.warn("APROVADOR TRES  NO FORMUALRIO ==> " + typeof hAPI.getCardValue("matApTres"))
		...
		log.warn(hAPI.getCardValue("matApDois") == "" || hAPI.getCardValue("matApDois") == null || hAPI.getCardValue("matApDois") == undefined || hAPI.getCardValue("matApDois") == "null")
			
		if(hAPI.getCardValue("matApDois") == "" || hAPI.getCardValue("matApDois") == null || hAPI.getCardValue("matApDois") == undefined){
		...
		log.warn(hAPI.getCardValue("matApTres") == "" || hAPI.getCardValue("matApTres") == null || hAPI.getCardValue("matApTres") == undefined || hAPI.getCardValue("matApDois") == "null")
		
		if(hAPI.getCardValue("matApTres") == "" || hAPI.getCardValue("matApTres") == null || hAPI.getCardValue("matApTres") == undefined){
		...
		var xml = "";
			xml += "<RhuReqAumentoQuadro>"
				xml += "<VReqAumentoQuadro>"
					xml += "<CODCOLREQUISICAO>"+hAPI.getCardValue("codColigada")+"</CODCOLREQUISICAO>"
					xml += "<IDREQ>-1</IDREQ>"
					xml += "<JUSTIFICATIVA>"+hAPI.getCardValue("justificativa")+"</JUSTIFICATIVA>"
				...
				xml += "<VReqAumentoQuadroCompl>"
					xml += "<CODCOLREQUISICAO>"+hAPI.getCardValue("codColigada")+"</CODCOLREQUISICAO>"
					xml += "<IDREQ>-1</IDREQ>"
					xml += "<REQTIPCONT>"+hAPI.getCardValue("codTipoContrato")+"</REQTIPCONT>"
					...
					xml += "<REQHABILIDADE>"+hAPI.getCardValue("habilidades")+"</REQHABILIDADE>"
		...
		var datasetPut = DatasetFactory.getDataset("dsPutAumentoQuadro", [xml], null, null);
